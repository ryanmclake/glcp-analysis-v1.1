...
basin <- list.files(path = "./output/slopes/basins")
basin <- gsub("\\..*", "", basin)

analysis_function <- function(x){

  system.time(d <- read_csv_arrow(
    paste0("./output/slopes/basins/",basin[1],".csv"),
    quote = "\"",
    escape_double = TRUE,
    ...
    timestamp_parsers = NULL) %>% rename(year = f0,
                                         ...
                                         ice_cover_count=f47,
                                         snow_km2=f48) %>%
      group_by(year, hybas_id, centr_lat, centr_lon, lake_type, depth_avg, elevation) %>%
      collect() %>%
      ...
      ungroup(.)%>%
      mutate(across(c(9:13), ~ scale(.))) %>%
      mutate(across(c(9:13), ~ if_else(is.na(.),0,.))) %>%
      select(-year) %>%
      group_by(hylak_id, hybas_id, centr_lat, centr_lon, lake_type, depth_avg, elevation) %>%
      summarise(across(c(1:5),  ~list(MannKendall(.) %>%
                                        tidy %>%
                                        select(p.value, statistic)))) %>%
      ungroup(.)%>%
      unnest(c(8:12), names_repair = "minimal") %>%
      collect() %>%
      write.table(., file = paste0("./output/slopes/hylak_id_slopes6.csv"),
                  append = T,
                  row.names = F,
                  col.names = !file.exists("./output/slopes/hylak_id_slopes6.csv")))
}
